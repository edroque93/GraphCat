...
parser = argparse.ArgumentParser(description='Create a random random graph.')
...
parser.add_argument('-i', action='store', default=10, dest='iters', type=int, help='iterations')
...
# Init
for r in range(0, args.iters):
    x,y = numpy.random.randint(0, args.size-args.win+1, 2)
    for i in range(x,x+args.win):
        for j in range(y,y+args.win):
